...
import { createBill } from "../../AllStateContainer/Bill/BillSlice";
...
export default function CreateBill() {
  ...
  const {
    ...
    defaultValues: {
      ...
      totalStudents: 0,
      presentStudents: 0, // Added missing field
      absentStudents: 0, // Added missing field
      totalBatches: 1, // Added missing field
      durationPerBatch: 1, // Added missing field
      balancePayable: 0,
      amountInWords: "",
  ...
  return (
    <div className="max-w-6xl mx-auto mt-6 p-6 shadow-lg rounded-2xl bg-white">
      <h2 className="text-2xl font-bold mb-6">Create Staff Payment Bill</h2>
      <form
      ...
      >
        ...
        <div>
          ...
          <input
            {...register("examSession", { required: "Exam session is required" })}
        ...
        <div>
          ...
          {errors.totalStudents && (
            <p className="text-red-500 text-sm">{errors.totalStudents.message}</p>
          )}
        </div>
          ...
          render={({ field }) => (
            <div>
              ...
              {errors.examStartTime && (
                <p className="text-red-500 text-sm">{errors.examStartTime.message}</p>
              )}
            </div>
          )}
        />
        ...
        <Controller
          ...
          render={({ field }) => (
            <div>
              ...
              {errors.examEndTime && (
                <p className="text-red-500 text-sm">{errors.examEndTime.message}</p>
              )}
            </div>
          )}
        />
        ...
        {/* Duration Per Batch */}
        <div>
          <label className="block mb-1 font-medium">Duration Per Batch (hours)</label>
          <input
            type="number"
          ...
          {errors.durationPerBatch && (
            <p className="text-red-500 text-sm">{errors.durationPerBatch.message}</p>
          )}
        </div>
              ...
              >
                <div className="grid grid-cols-3 gap-4 items-end">
                  <div>
                    ...
                    <input
                      {...register(`batches.${index}.batchNo`, { required: "Batch name is required" })}
                      placeholder="Batch 1"
                      className="border rounded px-2 py-1 w-full"
                    />
                    {errors.batches?.[index]?.batchNo && (
                      <p className="text-red-500 text-sm">{errors.batches[index].batchNo.message}</p>
                    )}
                  </div>
                  <div>
                    <label className="block mb-1 font-medium text-sm">
                    ...
                    {errors.batches?.[index]?.studentsPresent && (
                      <p className="text-red-500 text-sm">{errors.batches[index].studentsPresent.message}</p>
                    )}
                  </div>
          ...
          {staffFields.map((staff, sIndex) => (
            ...
            >
              ...
              <div className="mb-3">
                ...
                <select
                  {...register(`staffPayments.${sIndex}.role`, { required: "Role is required" })}
                ...
                {errors.staffPayments?.[sIndex]?.role && (
                  <p className="text-red-500 text-sm">{errors.staffPayments[sIndex].role.message}</p>
                )}
              </div>
        ...
        <div>
          ...
          {errors.balancePayable && (
            <p className="text-red-500 text-sm">{errors.balancePayable.message}</p>
          )}
        </div>
        <div className="col-span-2">
          <label className="block mb-1 font-medium">Amount in Words</label>
          <input
            {...register("amountInWords", { required: "Amount in words is required" })}
            readOnly
            placeholder="Five Thousand Only"
            className="w-full border rounded px-3 py-2 bg-gray-100"
          />
          {errors.amountInWords && (
            <p className="text-red-500 text-sm">{errors.amountInWords.message}</p>
          )}
        </div>
